...
ru_dictionary = FastVector(vector_file='wiki.hi.vec')
fr_vector = fr_dictionary["fine"]
ru_vector = ru_dictionary["महीन"]
print(FastVector.cosine_similarity(fr_vector, ru_vector))
# Result should be 0.02
fr_dictionary.apply_transform('alignment_matrices/en.txt')
ru_dictionary.apply_transform('alignment_matrices/hi.txt')
print(FastVector.cosine_similarity(fr_dictionary["chat"], ru_dictionary["बात"]))
# Result should be 0.43

sum_e = np.zeros((300, ))
sum_h = np.zeros((300, ))

for i in splitting_eng :
    g = i.lower()
    if fr_dictionary.__contains__(i)==False:
        continue
    else:
        sum_e = sum_e + fr_dictionary[g]
for i in splitting_hindi :
    if ru_dictionary.__contains__(i)==False:
        continue
    else:
        sum_h = sum_h + ru_dictionary[i]

print(FastVector.cosine_similarity(sum_e, sum_h))
